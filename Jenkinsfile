pipeline {
    ...
    environment {
        ...
        K8S_CLUSTER_NAME = 'muhannad-cluster'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/muhannadpv786/microservices-ingress-kastro.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|muhannadpv/techsolutions-app:latest|kastrov/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
